...
export default function ReviewersPage() {
  ...
  const institutionName = (id: string) => institutions.find((i) => i.id === id)?.name || 'Unknown'
    ...
    <div className="container mx-auto max-w-4xl space-y-6 px-4 py-8">
      <div className="space-y-1">
        <h1 className="flex items-center gap-2 text-2xl font-bold sm:text-3xl">
          <ShieldCheck className="h-7 w-7 text-bhutan-orange" /> Registration Reviewers
        </h1>
        <p className="text-sm text-muted-foreground">
          Assign who may approve or reject new registrations for each institution.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Only assigned reviewers (and superadmins) can approve registrations.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
          ...
          >
            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <><UserCheck className="mr-1 h-4 w-4" /> Assign</>}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {reviewers.length === 0 && (
            <p className="py-6 text-center text-sm text-muted-foreground">No reviewers assigned yet.</p>
          )}
          {reviewers.map((r) => (
            <div
              key={r.id}
            ...
            >
              <div className="min-w-0">
                <p className="truncate font-medium">{r.profile?.full_name || r.profile?.email || r.user_id}</p>
                <div className="mt-1 flex items-center gap-2">
                  <Badge variant="outline" className="text-xs">{institutionName(r.institution_id)}</Badge>
                  {r.profile?.role && <Badge variant="secondary" className="text-xs">{r.profile.role}</Badge>}
                </div>
              </div>
              <Button
                variant="ghost"
