...
-- ============================================================
-- This includes all tables and updates needed for:
-- - Timestamp Notes (student video notes with timestamps)
-- - Announcements (instructor course announcements)
-- - Reviews (course reviews and ratings)
-- - Teacher Course Creation (instructor course management)
-- ============================================================
...
-- ============================================================
-- ANNOUNCEMENTS TABLE (UPDATE IF NEEDED)
-- ============================================================
...
-- ============================================================
-- First, update the existing notes table to ensure it has all required fields
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information.schema.columns
...
-- ============================================================
-- Ensure courses table has all needed fields for instructor features
DO $$
BEGIN
...
-- Sample reviews
INSERT INTO reviews (user_id, course_id, rating, comment, helpful_count, not_helpful_count)
SELECT
  auth.uid(),
  c.id,
  (ARRAY[5,4,5,4,5])[floor(random() * 5 + 1)],
  (ARRAY['Excellent course!', 'Great content', 'Very helpful', 'Good explanations', 'Amazing instructor'])[floor(random() * 5 + 1)],
  floor(random() * 50),
  floor(random() * 5)
FROM courses c
LIMIT 3
...
INSERT INTO announcements (title, content, priority, course_id, is_global, is_pinned, created_by)
SELECT
  'Welcome to the Course!',
  'We are excited to have you join us. Get ready for an amazing learning journey!',
  'medium',
  c.id,
  false,
  true,
...
SELECT 'All new features schema created successfully!' as result;